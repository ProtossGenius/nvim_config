...
- Run the Java integration spec: `NVIM_TEST_JAVA_PROJECT=~/workspace/test-java nvim --headless -u "$PWD/init.lua" +"lua dofile('$PWD/test/java_file_actions_integration.lua')" +qa!`
...
- When changing Java rename behavior, use `user.file_actions` instead of direct filesystem renames. The Java path depends on JDTLS `textDocument/rename` plus `workspace/didRenameFiles` so class names and references are updated semantically.
